...
const ReverseCurvature: React.FC = () => {
    ...
    // Component for Venturi Flow Animation
      ...
      const transitionStart = -0.35; // 20% of the device width
      const transitionEnd = 1.15;   // 80% of the device width
...
const baseTopWidth = width;
const baseBottomWidth = baseTopWidth + width * 0; // Slightly longer bottom base
const baseHeight = height * 0.08; // 8% of device height for the base
...
const topCurveX = startX + width * 0.45;
const topCurveY = endY + height * 0.36;
const bottomCurveX = endX - width * 0.2;
const bottomCurveY = startY - height * 0.25;

// Calculate middle inward curve point
const middleX = startX + width * 0.5; // Adjust this to control how much it goes inside
const middleY = startY - height * 0.4; // Middle of the height
...
// Draw top curve
ctx.quadraticCurveTo(topCurveX, topCurveY, middleX, middleY);

// Draw bottom curve
ctx.quadraticCurveTo(bottomCurveX, bottomCurveY, endX, startY);
...
ctx.beginPath();
ctx.rect(startX, canvas.height - baseHeight, baseBottomWidth, baseHeight);
ctx.fill();
...
ctx.rect(
    ...
    width, // Use width instead of deviceWidth
    baseHeight
);
ctx.fill();
      ...
      return (
        <div className="w-full max-w-3xl mx-auto p-4 border rounded-lg shadow-md">
          <h2 className="text-xl font-bold mb-4">Reverse Curvature Flow Animation</h2>
...
export default ReverseCurvature;