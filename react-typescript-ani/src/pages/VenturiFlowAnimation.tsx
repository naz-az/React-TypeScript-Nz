...
const VenturiFlowAnimation6: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      // Draw Venturi device
      ctx.fillStyle = '#888';
      ctx.fillRect(200, canvas.height - 140, 100, 140);
      ctx.fillRect(225, canvas.height - 120, 50, 120);

      let maxWaterHeight = 0;

      if (waterPresent) {
        // Create water gradient with darker blue colors
        ...
        for (let x = 0; x < canvas.width * animationProgress; x++) {
          ...
          if (x > 200 && x < 300) {
            y += 40 * Math.sin((x - 200) / 100 * Math.PI);
          }
          if (x > 300) {
            y = canvas.height - h1;
          }
        ...
        ctx.strokeStyle = 'rgba(100, 150, 255, 0.3)';  // Lighter blue highlight
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(0, canvas.height - 80);
        for (let x = 0; x < canvas.width * animationProgress; x++) {
      ...
      gaugePositions.slice(0, numberOfGauges).forEach((position, index) => {
        ...
        // Draw Vernier scale marks with only 2 marks
        ctx.lineWidth = 2;
  ...
  }, [h1, waterPresent, animationProgress, gaugePositions, isDraining, drainProgress, isPaused, numberOfGauges]);
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4 border rounded-lg shadow-md">
      <h2 className="text-xl font-bold mb-4">Venturi Flow Animation</h2>
      <div className="mb-4">
        <label htmlFor="h1-input" className="block mb-2">Water Height (h1):</label>
        <input
          id="h1-input"
          type="number"
          value={h1}
...
export default VenturiFlowAnimation6;