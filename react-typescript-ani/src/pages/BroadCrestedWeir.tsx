...
const BroadCrestedWeir: React.FC = () => {
  const initialLeftWaterLevel = 130;
  const initialRightWaterLevel = 30;
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      // Dynamically calculate weir position
      const weirPosition = (canvas.width / 2) - (weirWidth / 2);
      ...
      if (waterPresent) {
        ...
        if (waterPosition > weirPosition && leftWaterLevel > weirHeight) {
          ...
          for (let i = 1; i <= numPoints; i++) {
            ...
            const x = weirPosition + 80 + waterfallWidth * t;
            const y = fallStart - 0 +(fallEnd - fallStart) * t * t;
            
            const waviness = 4 * Math.sin(t * Math.PI * 2 + time / 100) 
                           + 2 * Math.sin(t * Math.PI * 4 + time / 80);
            
            ctx.lineTo(x, y + waviness);
          }

          const cavityDepth =2 * waterfallProgress;
          ctx.lineTo(weirPosition + waterfallWidth * waterfallProgress, canvas.height - rightWaterLevel + cavityDepth + 60);
          ctx.quadraticCurveTo(
            weirPosition + 200+ (waterfallWidth * waterfallProgress) / 2, (canvas.height - weirHeight + cavityDepth * 6) - 30,
            weirPosition - 100, canvas.height - weirHeight + cavityDepth+10
          );
          ctx.lineTo(weirPosition, fallStart);
        ...
        if (waterPosition > weirPosition) {
          for (let x = weirPosition ; x < waterPosition; x++) {
            // weirWidth-90
      ...
      function drawRoundedTopRect(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, radius: number) {
        if (width < 2 * radius) radius = width / 2;
        ctx.beginPath();
        ctx.moveTo(x + radius, y);
        ctx.arcTo(x + width, y, x + width, y + radius, radius);
        ctx.arcTo(x + width, y + height, x + width - radius, y + height, 0);
        ctx.lineTo(x, y + height);
        ctx.arcTo(x, y + height, x, y + radius, 0);
      ...
      // Usage
      ctx.fillStyle = '#888';
      drawRoundedTopRect(ctx, weirPosition, canvas.height - weirHeight, weirWidth, weirHeight, 14);
  ...
  const handleRightWaterLevelChange = (e: ChangeEvent<HTMLInputElement>) => {
    setRightWaterLevel(Number(e.target.value));
  };
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4 border rounded-lg shadow-md">
      <h2 className="text-xl font-bold mb-4">Broad Crested Weir Animation</h2>
      <div className="mb-4">
        ...
        <div className="mt-4">
          <label htmlFor="right-water-level-input" className="block mb-2">Right Water Level:</label>
          <input
            id="right-water-level-input"
            type="number"
            value={rightWaterLevel}
            onChange={handleRightWaterLevelChange}
...
export default BroadCrestedWeir;