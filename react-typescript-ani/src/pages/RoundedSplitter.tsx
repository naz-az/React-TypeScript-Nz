import React, { useState, useEffect, useRef, ChangeEvent } from 'react';
  ...
  const RoundedSplitter: React.FC = () => {


    // Component for Venturi Flow Animation
      const [h2, setH2] = useState<number>(30); // State to manage water height
      ...
      const transitionStart = -0.5; // 20% of the device width
      const transitionEnd = 1.4;   // 80% of the device width
      ...
      useEffect(() => {
        ...
        const draw = (time: number) => {
          ...
          const throatEndX = throatStartX + throatWidth;
                    ...
                    const baseTopWidth = deviceWidth * 1.5;
                    ...
                    // Draw the main device body
                    ctx.fillStyle = deviceColor;
                    ctx.fillRect(deviceStartX, canvas.height - deviceHeight - baseHeight, deviceWidth, deviceHeight);
          
                    // Draw the throat
                    ctx.fillRect(throatStartX, canvas.height - throatHeight - baseHeight, throatWidth, throatHeight);
          
                    // Draw trapezium base
                    ctx.fillStyle = baseColor;
                    ctx.beginPath();
                    ctx.moveTo(deviceStartX - (baseTopWidth - deviceWidth) / 2, canvas.height - baseHeight);
                    ...
                    ctx.lineTo(deviceStartX - (baseBottomWidth - deviceWidth) / 2, canvas.height);
                    ctx.closePath();
                    ctx.fill();
      ...
      return (
        <div className="w-full max-w-3xl mx-auto p-4 border rounded-lg shadow-md">
          <h2 className="text-xl font-bold mb-4">Rounded Splitter Flow Animation</h2>
...
export default RoundedSplitter;