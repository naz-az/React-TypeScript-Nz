...
const SquareEdge: React.FC = () => {
  const [h2, setH2] = useState<number>(40);
    ...
    const draw = (time: number) => {
      ...
      if (waterPresent) {
        ...
        for (let x = 0; x < canvas.width * animationProgress; x++) {
          ...
          } else if (x <= deviceEndX +50) { // Extended the middle part by 50 pixels
            // Middle part
            t = (x - deviceStartX) / (deviceEndX + 50 - deviceStartX);            
            y = deviceStartY + deviceHeight - hm + (hm - h2) * Math.pow(t, 10);
          } else {
            // Third part
      ...
      ctx.arcTo(
        ...
        deviceEndX - cornerRadius, 
        deviceStartY + deviceHeight, 
        cornerRadius
      );
      
      ctx.lineTo(throatStartX, deviceStartY + deviceHeight);
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4 border rounded-lg shadow-md">
      <h2 className="text-xl font-bold mb-4">Square Edge</h2>
