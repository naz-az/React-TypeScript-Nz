...
const NonModularCrumpWeir: React.FC = () => {
  const [leftWaterHeight, setLeftWaterHeight] = useState<number>(100);
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      // Draw extremely extended Crump weir
      const weirHeight = 80;
      const weirWidth = 60;
      const weirCenterX = canvas.width / 2 - 40;
      const weirY = canvas.height - weirHeight;

      // Calculate extended base points
      const leftExtension = weirWidth * (0.5);
      const rightExtension = weirWidth * (2.5);
      const leftBaseX = weirCenterX - weirWidth / 2 - leftExtension;
      const rightBaseX = weirCenterX + weirWidth / 2 + rightExtension;

      // Draw the weir
      ctx.fillStyle = 'orange';
      ctx.beginPath();
      ctx.moveTo(leftBaseX, canvas.height);
      ctx.lineTo(weirCenterX, weirY);
      ...
      if (waterPresent) {
        ...
        const rightWaterHeight = leftWaterHeight * 0.4; // 30% less than left water height
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4 border rounded-lg shadow-md">
      <h2 className="text-xl font-bold mb-4">Non-Modular Crump Weir Flow Animation</h2>
      <div className="mb-4">
...
export default NonModularCrumpWeir;