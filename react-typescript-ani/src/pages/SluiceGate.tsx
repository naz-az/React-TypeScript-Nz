...
const SluiceGate: React.FC = () => {
  const [h2, setH2] = useState<number>(0); // State to manage water height
    ...
    const draw = (time: number) => {
      ...
      if (waterPresent) {
        ...
        for (let x = 0; x < canvas.width * animationProgress; x++) {
          let y = deviceStartY + deviceHeight - 80 - h1;

          // Middle Part
          if (x > deviceStartX && x < deviceEndX) {
            y = deviceStartY + 10 - hm;
          }
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4 border rounded-lg shadow-md">
      <h2 className="text-xl font-bold mb-4">Sluice Gate</h2>
