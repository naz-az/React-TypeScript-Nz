...
const ChangeWaterDepth: React.FC = () => {

  

  const [h2, setH2] = useState<number>(35); // State to manage water height
  ...
  const transitionStart = -0.4; // 20% of the device width
  const transitionEnd = 2.7;   // 80% of the device width
    ...
    const draw = (time: number) => {
      ...
      if (waterPresent) {
        ...
        for (let x = 0; x < canvas.width * animationProgress; x++) {
          ...
          y += 4 * Math.sin((canvas.width - x) / 30 - time / 300)
...
const lightBaseColor = '#888'; // Lighter color for the rectangular part

// Calculate trapezium base dimensions
const baseTopWidth = 200;
const baseBottomWidth = baseTopWidth + deviceWidth * 1.2; // Slightly longer bottom base
const baseHeight = 20; // 20% of device height for the base

// Draw trapezium base
ctx.fillStyle = baseColor;
ctx.beginPath();
ctx.moveTo(deviceStartX - (baseBottomWidth - deviceWidth) / 2, canvas.height);
ctx.lineTo(deviceStartX + deviceWidth + (baseBottomWidth - deviceWidth) / 2, canvas.height);
ctx.lineTo(deviceStartX + deviceWidth + (baseTopWidth - deviceWidth) / 2, canvas.height - baseHeight);
ctx.lineTo(deviceStartX - (baseTopWidth - deviceWidth) / 2, canvas.height - baseHeight);
ctx.closePath();
ctx.fill();

// Draw lighter rectangular part
ctx.fillStyle = lightBaseColor;
ctx.fillRect(
  deviceStartX - (baseTopWidth - deviceWidth) / 2,
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4 border rounded-lg shadow-md">
      <h2 className="text-xl font-bold mb-4"> Flume Flow over Changing Water Depth</h2>
...
export default ChangeWaterDepth;